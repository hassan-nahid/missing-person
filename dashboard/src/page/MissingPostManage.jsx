...
const MissingPostManage = () => {
  const [search, setSearch] = useState("");
  const [missingData, setMissingData] = useState([]); // Store missing data
  ...
  useEffect(() => {
    const fetchMissingData = async () => {
      try {
        ...
        setMissingData(data?.data || []); // Set the missing data
      } catch (error) {
  ...
  // Filter missing data based on the search query and foundStatus
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/missing/adminDelete/${id}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
        ...
        setMissingData((prevData) => prevData.filter((item) => item._id !== id));
    ...
    <div className="overflow-x-auto">
      <div className="container mx-auto py-8">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold blue-text">Missing Post</h1>
          <p className="text-lg text-gray-600 mt-2">
            Browse and manage missing persons data.
          </p>
        </div>
              ...
              filteredData.reverse().map((item, index) => (
                <tr key={item._id}>
                  ...
                  <td className="flex gap-2">
                    <Link to={`/admin_missing_details/${item._id}`} className="btn btn-info text-white btn-xs">Details</Link>
                    <button
...
export default MissingPostManage;
